...
class BoxCollider(Collider):
    def __init__(self, body, half_extents):
        super().__init__(body)
...
class ConvexHullCollider(Collider):
    def __init__(self, body, local_verts):
        super().__init__(body)
...
def line_case(s, d):
    ...
    if ab.dot(ao) > 0:
        d.x, d.y, d.z = triple_product(ab, ao, ab).x, triple_product(ab, ao, ab).y, triple_product(ab, ao, ab).z
    else:
        s.set([a])
...
def epa(c1, c2, simplex, max_iter=64, tol=1e-4):
    if len(simplex) < 4:
    ...
    if len(simplex) < 4:
        center = (simplex[0] + simplex[1] + simplex[2]) * (1.0/3.0)
        n = Vec3(0, 1, 0)
        return n, 0.01
...
class Contact:
    def __init__(self, pt, normal, depth, body_a, body_b):
        self.pt = pt
        self.normal = normal
        self.depth = depth
        self.body_a = body_a
...
def find_contact_point(c1, c2, normal, depth):
    pt_a = c1.support(normal)
    pt_b = c2.support(-normal)
    return (pt_a + pt_b) * 0.5


def detect_collision(c1, c2):
...
class GroundPlane:
    def __init__(self, y=0, normal=None):
        self.y = y
        self.normal = normal if normal else Vec3(0, 1, 0)


def detect_ground_collision(collider, ground):
...
if __name__ == "__main__":
    ...
    if contact:
        ...
        print(f"  Point: {contact.pt}")
    else:
        print("No collision")
    
    print("\n=== GJK Box-Sphere Test ===")
    b3 = RigidBody(mass=1.0, pos=Vec3(0, 0, 0))
    b4 = RigidBody(mass=1.0, pos=Vec3(1.3, 0, 0))
    
    box = BoxCollider(b3, Vec3(0.5, 0.5, 0.5))
    sphere = SphereCollider(b4, 1.0)
    
    contact = detect_collision(box, sphere)
    if contact:
        print(f"Collision detected!")
        print(f"  Normal: {contact.normal}")
        print(f"  Depth: {contact.depth:.4f}")
    ...
    contact = detect_ground_collision(s5, ground)
    if contact:
        print(f"Ground collision!")
        print(f"  Depth: {contact.depth:.4f}")
        print(f"  Point: {contact.pt}")
    else:
        print("No ground collision")
    
    print("\n=== No Collision Test ===")
    b6 = RigidBody(mass=1.0, pos=Vec3(0, 0, 0))
    b7 = RigidBody(mass=1.0, pos=Vec3(5, 0, 0))
    
    s6 = SphereCollider(b6, 1.0)
    s7 = SphereCollider(b7, 1.0)
    
    contact = detect_collision(s6, s7)
    print(f"Distant spheres colliding: {contact is not None}")
